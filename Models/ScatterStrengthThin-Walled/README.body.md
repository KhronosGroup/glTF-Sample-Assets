...
## Description
This asset demonstrates the effect of varying scatter strength and transmission strength on a thin-walled material. Each shader ball uses the `KHR_materials_scatter` extension with values for `scatterStrengthFactor` ranging from 0 to 1. One column uses `scatterStrengthTexture` to show fragment-varying strength.
